...
const NOISE_TITLE_RE = new RegExp(
    [
        ...
        "\\bmixtape\\b", "\\bcontinuous\\s+mix\\b", "\\bfull\\s+album\\b",
        "\\bfull\\s+mix\\b", "\\bgreatest\\s+hits\\b",
        "\\bvol\\.?\\s*\\d", "\\bvolume\\s*\\d", "\\bbest\\s+of\\b",
        "\\btop\\s*\\d+\\b", "\\b\\d+\\s*hour", "\\b\\d+\\s*min\\b",
        // Party-mix variants (e.g. "Wiz Party", "Party Mix", "Party Mixtape")
        "\\bwiz\\s+party\\b", "\\bparty\\s+(mix|mixtape)\\b",
