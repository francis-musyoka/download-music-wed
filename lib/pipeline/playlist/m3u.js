...
function generateM3U({ name, tracks, outputDir = PLAYLISTS_DIR }) {
  ...
  // Use the sanitized name in the header too — the raw `name` could contain
  // newline/control chars that would inject extra lines into the M3U body.
  let content = "#EXTM3U\n";
  content += `#PLAYLIST:${safeName}\n\n`;
