...
function downloadTrack(url, outputDir = MUSIC_DIR) {
    return new Promise((resolve, reject) => {
        ...
        const args = [
            // Force IPv4. On datacenter hosts (e.g. Contabo) YouTube binds the
            // stream URL to the requesting IP and 403s the data fetch when it's
            // an IPv6 address in a flagged range — extraction succeeds but the
            // byte download fails. Going out over IPv4 sidesteps the block.
            "--force-ipv4",
            "-x",
            "--audio-format",
            "mp3",
