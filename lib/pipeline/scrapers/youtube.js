...
function downloadTrack(url, outputDir = MUSIC_DIR) {
    return new Promise((resolve, reject) => {
        ...
        const args = [
            ...
            // Force YouTube's Android-VR API client. Default behavior probes
            // multiple clients (web → web_safari → ios → ...) and burns ~10s
            // on slow ones running the JS signature solver before falling
            // through to android_vr. Pinning it skips the probing entirely.
            // Pairs with the same flag in lib/preview-resolver.ts.
            "--extractor-args",
            "youtube:player_client=android_vr",
            // Opt in to fetching the EJS (challenge solver) script from
            // yt-dlp's own GitHub release. Kept as defense-in-depth in case
            // android_vr is ever gated and yt-dlp needs to fall back to a
            // client that requires JS challenge solving. Pairs with Deno on
            // PATH.
            "--remote-components",
            "ejs:github",
            "-o",
