...
async function scrapeYTMusicSearch(page, query) {
    try {
        ...
        try {
            // CRITICAL: scope to shelf rows. YT Music keeps ~2 invisible
            // `ytmusic-responsive-list-item-renderer` nodes in
            // `ytmusic-search-suggestions-section` (the autocomplete dropdown).
            // Playwright's default `state: "visible"` waits for the FIRST
            // match — and the first match in document order is the hidden
            // suggestion row, so the wait times out even when real results
            // are on screen. Scoping with the shelf-renderer ancestor skips
            // the suggestion nodes entirely. Verified with scripts/trace-yt-music.js.
            await page.waitForSelector(
                "ytmusic-shelf-renderer ytmusic-responsive-list-item-renderer",
                { timeout: 12_000 },
            );
        } catch {
            // No real shelf results rendered in time — let the subsequent
            // evaluate() return [].
            return [];
        }

        // Try to click "Songs" filter — but don't fail if it's not visible.
        // The chip-cloud mounts *after* the result rows on slower upstreams
        // (we have measured this with scripts/trace-yt-music.js — locally the
        // chip cloud appears ~1–2s after the first list-item renderer). The
        // previous code probed `page.$()` immediately after the row wait and
        // returned null because the cloud hadn't hydrated yet → "chip not
        // found in DOM". Wait for the chip-cloud renderer first, then probe.
        // Note: `text=Songs` (substring) is intentionally NOT used — it
        // greedily matches the bold search-term echo on the results page
        // ("burna boy popular **songs**") whose click never resolves.
        try {
            await page.waitForSelector("ytmusic-chip-cloud-renderer", { timeout: 8000 });
            ...
            // After the filter click, the list re-renders into a "Songs"
            // shelf. Same suggestion-row gotcha as the initial wait — scope
            // to shelf rows so we don't get stuck on the invisible
            // suggestion nodes.
            await page.waitForSelector(
                "ytmusic-shelf-renderer ytmusic-responsive-list-item-renderer",
                { timeout: 5_000 },
            );
            // Rows hydrate progressively: trace measured shelf count growing
            // 20 → 40 → 60 rows over ~4s. Worse, the SUBTITLE column within
            // each row populates even later — at extraction time we were
            // getting rows where parts ended at "Album" with no duration
            // trailer, so parseSubtitle yielded durationStr="" → 0s on every
            // result. Wait until at least 9 rows have a duration-shaped tail
            // in their second flex column, then extract.
            await page
                .waitForFunction(
                    () => {
                        ...
                        for (const row of rows) {
                            const cols = row.querySelectorAll(
                                ".flex-columns yt-formatted-string",
                            );
                            const sub = cols[1] && cols[1].textContent
                                ? cols[1].textContent.trim()
                                : "";
                            if (/\d+:\d{2}(?::\d{2})?$/.test(sub)) withDuration++;
                            if (withDuration >= 9) return true;
                        }
                        return false;
                    },
                    { timeout: 5_000 },
        ...
        const rawResults = await page.evaluate(() => {
            ...
            // When scoped to the Songs shelf, query its rows directly. When not
            // (filter failed or no Songs shelf), still skip the invisible
            // `ytmusic-search-suggestions-section` rows by scoping to any shelf.
            const rows = scope
                ? scope.querySelectorAll("ytmusic-responsive-list-item-renderer")
