...
async function scrapeYTMusicSearch(page, query) {
  try {
    ...
    try {
      ...
      // Songs filter not available or didn't re-render — continue with mixed results.
    }
...
async function collectCandidates(genre, opts = {}) {
  ...
  try {
    ...
    const searchQueries = [
      `${genre} hits`,
      `${genre} top songs ${year}`,
      `best ${genre} songs`,
      `${genre} new releases ${year}`,
      `trending ${genre}`,
      ...(config.searchTerms ?? []),
      ...extraSearchTerms,
    ];

    // Deduplicate queries and cap at MAX_SCRAPE_QUERIES
    const uniqueQueries = [...new Set(searchQueries)].slice(0, MAX_SCRAPE_QUERIES);
    ...
    const queueCopy = uniqueQueries.slice();
    ...
    async function worker(workerIdx) {
      ...
      try {
        while (queueCopy.length > 0) {
          ...
          const idx = ++completed;
          log.info(`Search ${idx}/${uniqueQueries.length}: "${query}"`);
