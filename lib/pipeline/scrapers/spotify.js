  ...
  try {
    ...
    for (let i = 0; i < uniqueQueries.length; i++) {
      if (Date.now() > deadline) {
        log.warn(
          `Scraper budget exceeded after ${i}/${uniqueQueries.length} queries; returning partial results`,
        );
        break;
      }
...
async function collectArtistCandidates(artist) {
  ...
  try {
    ...
    for (let i = 0; i < searchQueries.length; i++) {
      if (Date.now() > deadline) {
        log.warn(
          `Scraper budget exceeded after ${i}/${searchQueries.length} queries; returning partial results`,
        );
        break;
      }
