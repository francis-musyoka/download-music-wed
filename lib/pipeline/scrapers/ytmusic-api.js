...
// number of internal pages); real calls land in 3–12s. 20s catches hangs
// without making the all-3-fail worst case feel like the process is dead.
...
async function searchSongs(query, limit, opts = {}) {
    ...
    return new Promise((resolve, reject) => {
        execFile(
            ...
            (err, stdout, stderr) => {
                if (err) {
                    // Exit code 3 + "ytmusicapi" in stderr = the sidecar's Python
                    // doesn't have ytmusicapi importable. This is the silent-hang
                    // failure mode from a misconfigured deploy (missing
                    // YTMUSIC_PYTHON or unbuilt venv). Surface it as an actionable
                    // error within milliseconds instead of waiting on the timeout.
                    if (err.code === 3 && /ytmusicapi/i.test(stderr || "")) {
                        reject(new Error(
                            `ytmusicapi is not importable from ${PYTHON_BIN}. ` +
                            `Set YTMUSIC_PYTHON to a Python with ytmusicapi installed ` +
                            `(create the venv with: python3 -m venv .venv-ytmusic && ` +
