...
First-time setup needs `npx playwright install chromium` — `npm install` does not auto-fetch the browser. System binaries on PATH: `yt-dlp`, `ffmpeg`, `spotdl` (optional). The app degrades gracefully without `OPENAI_API_KEY` but quality drops noticeably.
...
app/page.tsx ──POST──> /api/{rank,download} ──> lib/pipeline/orchestrator.ts
                                                   │
                                                   ├─ scrapers/{spotify,youtube}.js  (Playwright)
                                                   ├─ enrich/uploadDates.ts          (yt-dlp)
                                                   ├─ scoring/{noiseFilter,hitScore,diversity}.js
                                                   └─ llm/{understandQuery,rerankCandidates}.ts

app/page.tsx ──GET (SSE)──> /api/progress/[jobId] ──> lib/jobs.ts subscribers
```

Job state, rate buckets, preview cache, and the understand-cache all live in process memory — no database, no Redis. They are hoisted onto `globalThis.__downloadMusic*` keys so Next.js dev HMR doesn't reset them. Process restart is the only thing that clears them. See "globalThis hoisting" below.
...
11. **Spotify path** is primary when `SPOTIFY_CLIENT_ID`+`SECRET` are set. Auto fall back to YT Music scrape when results are thin or Spotify throws. See `docs/superpowers/specs/2026-05-20-spotify-integration-design.md`.
...
- **No Jest / Vitest.** `node --test` with `--experimental-strip-types`. New unit tests go under `lib/**/__tests__/*.test.ts`.
...
If you edit a prompt or LLM logic and want to retry a previously-resolved query, **restart the dev server** — `understandCache` memoizes successful resolutions until process exit. Likewise for the preview cache and job map; HMR will preserve them, only a full restart clears them.
