...
import { completeJob, createJob, emit, failJob, getJob } from "@/lib/jobs";
import {
    checkRate,
    checkOverallDaily,
    checkExpensiveDaily,
...
export async function POST(req: Request) {
    ...
    if (heavy) {
        const overallRetry = checkOverallDaily(sessionId);
        if (overallRetry !== null) {
            return errorResponse(
                429,
                {
                    error: "Daily search limit reached. Try again tomorrow.",
                    retryAfter: overallRetry,
                },
                overallRetry,
            );
        }
        const expensiveRetry = checkExpensiveDaily(sessionId);
        if (expensiveRetry !== null) {
            return errorResponse(
                429,
                {
                    error: "Daily search limit reached. Try again tomorrow.",
                    retryAfter: expensiveRetry,
                },
                expensiveRetry,
            );
        }
    ...
    if (heavy) incrementOverallDaily(sessionId);
        ...
        runner()
            .then(({ tracks, note }) => {
                if (heavy) {
                    ...
                    if (j?.expensiveLLMFired) incrementExpensiveDaily(sessionId);
                }
                completeJob(jobId, tracks, { note });
            })
            .catch((err: unknown) => {
